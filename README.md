...
- Create a clean zone of configurable size around the spawn point.
  ...
  - Optionally change the floor to a single tile type for the whole area.
    ...
    - Choose whether to only replace land, or water as well.
      - If fish are not removed, water tiles holding these will be kept regardless of this setting, leaving some tiny puddles if the rest of the lake __is__ removed.
...
Some features may feel like cheating, but I learned an important lesson a while ago: there is no right or wrong way to play games. Adjusting the single player mode doesn't affect anyone else in Factorio and as you bought it, you have the right to enjoy it the way __you__ want. You don't have to stick to how the creators intended it or how others prefer it.